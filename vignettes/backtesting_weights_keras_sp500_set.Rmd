...
config_unlimited_sharpe <- list(
  ...
  callbacks = list(
   callback_early_stopping(monitor = "loss", min_delta = -0.0001, patience = 10) #patience = 10 originally
   ),
  epochs = 60,
  batch_size = 256, #originally 128
  verbose = 1,
  seeds = c(1,19), #(1,13,19,42,57)
  plot_training = FALSE,
  plot_result = FALSE
)
...
model_portfolios <- keras_portfolios9_bigger_net

# Summarize portfolio weights
tt <- summary.weights(model_portfolios, print=TRUE)
```
...
tt <- summary.performance2(model_portfolios, transaction_cost = 0.005, gamma = 3, test=TRUE, print=TRUE)
```
...
keras_portfolios1_pp <- postprocessing_portfolios(model_portfolios, pp_config)

# Summarize postprocessed weights
tt <- summary.weights(keras_portfolios1_pp, print=TRUE)
```
...
tt <- summary.performance2(keras_portfolios1_pp, transaction_cost = 0.005, gamma = 5, test=TRUE, print=TRUE)
```
