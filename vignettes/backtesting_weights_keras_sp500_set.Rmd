...
I did an initial environment setup to ensure reproducible, performant training and analysis:

* Configured TensorFlow’s threading (intra-op and inter-op) so matrix operations and independent graphs run on a controlled number of CPU cores, avoiding unpredictable parallelism.
* Hooked into my Python “r-reticulate” virtualenv via **reticulate**, verified that **tensorflow** and **keras** modules are present, and printed key BLAS/OpenMP thread‐count environment variables for transparency.
* Loaded **keras3** for defining and training neural networks, then brought in **dplyr**, **tibble**, and **PerformanceAnalytics** for data wrangling and financial performance metrics.
* Finally, used **devtools::load\_all()** to load my **InvestigatoR** package (as defined in *InvestigatoR-package.R*), which provides the core functions for backtesting portfolios (e.g. in *backtesting\_portfolios.R*) and return/weight prediction (e.g. *return\_pred\_functions2.R*, *weight\_pred\_functions.R*).

This primes the session so that in the next steps I can seamlessly build, train, and evaluate models—and then feed their outputs into my backtesting framework.
...
load(file = "/home/apischetsrieder/Thesis/InvestigatoR/data-raw/sp500_m_signals1.RData", verbose=TRUE)

```


I loaded the dataset.
The dataset contains 1,207 stocks with 93 features, including firm-specific attributes like valuation, profitability, momentum, risk, estimates, and liquidity. The data is not perfectly rectangular, reflecting a realistic financial environment.
...
sp500_m_signals <- sp500_m_signals1
...
sp500_m_signals %>% 
    dplyr::select(c(features, "ret", "date")) %>%     # Keep few features, label & date
    group_by(date) %>%                                          # Group: dates!
    summarise_all(funs(cor(.,ret))) %>%                     # Compute correlations
    dplyr::select(-ret) %>%                                 # Remove label
...
config_unconstrained_deep <- list(
  layers = list(
...
config_unconstrained_shallow <- list(
  layers = list(
    list(type = "dense", units = 32, activation = "relu",dropout = 0.3),
    list(type = "dense", units =  16, activation = "relu"),
    list(type = "dense", units = 1, activation = activation_box_sigmoid(min_weight = -1, max_weight = 1))  # Bound delta_w
  ),
  loss = list(
...
I set up a custom Keras model configuration to learn unconstrained portfolio weight adjustments that directly target risk-adjusted performance:

* **Architecture:** a three-layer feed-forward network (32 → 16 → 1 neurons) with ReLU activations in the hidden layers and a custom “box-sigmoid” output that ensures each predicted weight delta lies between –1 and +1.
* **Objective:** instead of a plain MSE, I’m optimizing an active returns loss (“information\_ratio\_loss\_active\_returns”) so the model learns weight shifts that boost the portfolio’s information ratio relative to the benchmark.
* **Regularization & Optimization:** no L₁/L₂ penalties, Adam optimizer at a 0.001 learning rate, and early-stopping on the loss (allowing up to 10 epochs without meaningful improvement).
* **Training Details:** 60 epochs, 256-sized batches, two fixed random seeds (1 and 19) for reproducibility, and no on-the-fly plotting.

With this `config_unlimited_sharpe` list, I can plug straight into my return-and-weight-prediction pipeline (in *return\_pred\_functions2.R* and *weight\_pred\_functions.R*), train the model to favor high-information-ratio moves, then hand its outputs off to the backtester (in *backtesting\_weights.R* and *backtesting\_portfolios.R*) for out-of-sample evaluation.
...
config_constrained_shallow <- config_unconstrained_shallow
config_constrained_shallow$layers[[3]]$activation <- activation_box_sigmoid(min_weight = -0.05, max_weight = 0.05)
#config_constrained_shallow$loss$transaction_costs <- 0.005

config_constrained_deep <- config_unconstrained_deep
config_constrained_deep$layers[[3]]$activation <- activation_box_sigmoid(min_weight = -0.05, max_weight = 0.05)
#config_constrained_deep$loss$transaction_costs <- 0.005

```

I set up the inputs and three different modeling regimes for learning portfolio weight shifts on the S\&P 500 signals, ready to feed into my prediction–backtest pipeline:

1. **Data & CV parameters**

   * Chose the one-month ahead return `"ret"` as the prediction target.
   * Pulled in my pre-computed `sp500_m_signals` object (from *SP500\_All.R*), which holds monthly features like dividend yield, earnings-to-price, 12-month momentum, idiosyncratic vol, investment, book-to-market, and a host of other factor signals.
   * Defined a stable feature set:
     features <- c("div_yield_st","ep","mom12m","idio_vol3f","investment",
                   "bm","am","fr","gp","ms","noa","ps","rd","ro_e","sp","vol_mkt")
   * Configured my rolling-window cross-validation: 5-year training windows stepping by 1 year, offset by one month for out-of-sample returns, and keeping all data in-sample for diagnostics.

2. **Unlimited Deviations**
   Kept the previously defined `config_unlimited_sharpe`—a 32→16→1 network with box-sigmoid output bound at ±1—so the model has free rein to tilt weights however it pleases to maximize information ratio.

3. **Limited Deviations (± 20%)**
   Cloned that config to `config_limited_sharpe` but tightened the last layer’s box-sigmoid bounds to ±0.2, so the network can only deviate each security’s weight by up to 20 percentage points.

4. **Regularized Deviations (± 20% + L₁/L₂)**
   Further extended `config_limited_sharpe` into `config_regularized_sharpe` by adding L₁ and L₂ penalties (both λ = 0.001) on the weight shifts, encouraging sparser, smaller moves.

With these three configurations defined, I can loop over them in my return-prediction functions (*return\_pred\_functions2.R*) and weight-prediction routines (*weight\_pred\_functions.R*), then hand off their output to the backtesting engines in *backtesting\_weights.R* and *backtesting\_portfolios.R* for a clean, apples-to-apples evaluation of unconstrained vs. constrained vs. regularized strategies.


Now we run the backtest:

```{r run_backtest, cache=TRUE}
# Run the backtest
pf_config_deep <- list(
  keras_constrained_deep = list(
    weight_func = "keras_weights_new",
    config = config_constrained_deep
  )
)
# Run the backtest with multiple Keras configurations
keras_portfolios_constrained_deep_bm <- backtesting_weights_new(
  data = sp500_m_signals,
  return_label = return_label,
  benchmark_label = "bm_weight",
  mask_label = "sp500_flag",
  features = features,
  pf_config = pf_config_deep,  # Portfolio config with multiple Keras configurations
...
save(keras_portfolios_constrained_deep_bm, file = "keras_portfolios_constrained_deep_bm.RData")

# Run the backtest
pf_config_deep_un <- list(
  keras_unconstrained_deep = list(
    weight_func = "keras_weights_new",
    config = config_unconstrained_deep
  )
)
# Run the backtest with multiple Keras configurations
keras_portfolios_unconstrained_deep_bm_new <- backtesting_weights_new(
  data = sp500_m_signals,
  return_label = return_label,
  benchmark_label = "bm_weight",
  mask_label = "sp500_flag",
  features = features,
  pf_config = pf_config_deep_un,  # Portfolio config with multiple Keras configurations
  # portfolio_object = keras_portfolios8,
  rolling = TRUE,
  window_size = window_size,
  step_size = step_size,
...
save(keras_portfolios_unconstrained_deep_bm_new, file = "keras_portfolios_unconstrained_deep_bm_new.RData")

# Run the backtest
pf_config_shallow <- list(
  keras_constrained_shallow = list(
    weight_func = "keras_weights_new",
    config = config_constrained_shallow
  )
)
# Run the backtest with multiple Keras configurations
keras_portfolios_constrained_shallow <- backtesting_weights_new(
  data = sp500_m_signals,
  return_label = return_label,
  benchmark_label = "bm_weight",
  mask_label = "sp500_flag",
  features = features,
  pf_config = pf_config_shallow,  # Portfolio config with multiple Keras configurations
  # portfolio_object = keras_portfolios8,
  rolling = TRUE,
  window_size = window_size,
  step_size = step_size,
...
save(keras_portfolios_constrained_shallow, file = "keras_portfolios_constrained_shallow_bm.RData")
...
# Run the backtest
pf_config_shallow_un <- list(
  keras_unconstrained_shallow = list(
    weight_func = "keras_weights_new",
    config = config_unconstrained_shallow
  )
)
# Run the backtest with multiple Keras configurations
keras_portfolios_unconstrained_shallow <- backtesting_weights_new(
  data = sp500_m_signals,
  return_label = return_label,
  benchmark_label = "bm_weight",
  mask_label = "sp500_flag",
  features = features,
  pf_config = pf_config_shallow_un,  # Portfolio config with multiple Keras configurations
  # portfolio_object = keras_portfolios8,
  rolling = TRUE,
  window_size = window_size,
  step_size = step_size,
...
save(keras_portfolios_unconstrained_shallow, file = "keras_portfolios_unconstrained_shallow_bm.RData")
...
I loaded my unlimited-deviation setup into the backtester and ran a full rolling-window evaluation:

* I created `pf_config1`, mapping a single strategy “keras\_unlimited\_bm\_sharpe” to my new Keras weight generator (`keras_weights_new`) paired with the `config_unlimited_sharpe` settings (±100% bounds, information-ratio loss, no regularization).
* I then invoked `backtesting_weights_new()` on the `sp500_m_signals` dataset—using one-month-ahead returns (`ret`), the benchmark weight (`bm_weight`), and my S\&P-500 stock mask—over 5-year training windows rolling forward by one year (with a one-month out-of-sample offset), parallelized over 10 cores.
* With `verbose=TRUE`, I watched the model train and generate portfolio weights for each fold, then aggregated their realized returns and risk metrics.
* Finally, I saved the resulting backtest object as `"keras_portfolios1-v2.RData"` for later analysis and comparison against the limited and regularized variants.
...
#Insert the the model to summarize

combined_portfolios <- combine_portfolioReturns(list(
  keras_portfolios_constrained_deep_bm,
  keras_portfolios_unconstrained_deep_bm_new,
  keras_portfolios_constrained_shallow,
  keras_portfolios_unconstrained_shallow
  ))

save(combined_portfolios, file = "combined_portfolios.RData")
...
I did a quantitative recap of how my unconstrained Sharpe-maximizing model actually positioned over time:

* **Extremes:**  Individual stock weights swung as low as –56 % and up to +102 %, confirming the network freely levered its bets.
* **Typical Tilt:**  The average weight per stock was only \~14 % (SD ≈ 19 %), with weight volatility across months of \~17 % (SD ≈ 5 %).
* **Concentration:**  The mean Herfindahl–Hirschman Index sat at \~56 (SD ≈ 46), indicating moderate concentration, and a diversity score of just 0.03.
* **Overall Activity:**  The average L₁ norm of weight changes was \~176 (SD ≈ 92), and the L₂ norm \~6.9 (SD ≈ 2.7), showing substantial portfolio turnover and tilt magnitude.
* **Short Exposure:**  On average, cumulative shorts summed to \~230 % of NAV (SD ≈ 134 %), illustrating heavy use of negative weights.

This summary (from `summary.weights(keras_portfolios1)`) confirms that the “unlimited” model indeed takes large, concentrated, and frequently changing positions—exactly what I’ll contrast against the ±20 % and regularized variants next.
...
```
**Leistungsvergleich analysierend**

Ich habe die Leistungsbewertung vorgenommen, einschließlich Transaktionskosten und Risikoneigung. Das Ergebnis zeigt vor den Kosten extrem hohe Renditen, eine hohe Umschlagsrate und einen nach Kosten angepassten Sharpe, der immer noch hoch bleibt. Der moderate Informationsverhältniswert deutet darauf hin, dass es eine gute Anpassung gibt, jedoch wirkt sich die risikoadjustierte CER negativ aus. Im Vergleich zum Benchmark gibt es eine deutliche Überperformance. Die Auswertung bezieht sich auf eine Testphase, daher sind die T-Tests für die Signifikanz relevant.


I did a full out-of-sample performance audit of the unconstrained (±100 %) model

and here’s what I found:

* **Gross vs. net returns:** before costs, the model delivered an annualized mean of **48.6 %** (highly significant) with a Sharpe of **1.91**; by contrast the cap-weighted benchmark returned just **2.8 %** with a Sharpe of **0.45**.
* **Turnover & active share:** it traded like mad—**\~47 465 %** turnover per year and an **81.8 %** active share—so I applied a **0.5 % round-trip cost**.
* **Cost-adjusted Sharpe:** even after fees, the net Sharpe only dipped to **1.78**, showing that despite the punishing churn, the strategy still packs a punch.
* **Information ratio:** versus the benchmark, the IR sits at **0.16**, again strongly significant.
* **CER (γ = 3):** computing a CRRA-style Certainty Equivalent Return gave **–917 %**, flagging that under a fairly high risk-aversion the extreme volatility and trading intensity produce negative utility.

This tells me that while the unconstrained network can uncover massive gross gains, its sky-high turnover and tail-risk make it potentially unviable for a risk-averse, cost-sensitive investor—exactly the motivation for testing my ±20 % and regularized variants next.
...
pp_config <- list(
  # 1. Clamp individual weights between 0 and 10%
  list(operation      = "set_weightsum",
       allow_short_sale= FALSE,
       min_weight      = 0,
       max_weight      = 0.10,
       # (optionally enforce full investment)
       sum_val         = 1
  ),
  # 2. Smooth turnover (exponential with factor 0.2)
  list(operation        = "reduce_turnover",
       method           = "exponential",
       smoothing_factor = 0.2
  ),
  # 3. Cap concentration (HHI target 0.3)
  list(operation = "increase_diversification",
       hh_target   = 0.3
  )
)
# Apply postprocessing

combined_portfolios_pp <- postprocessing_portfolios(combined_portfolios, pp_config)


# Summarize postprocessed weights

tt <- summary.weights(combined_portfolios_pp, print=TRUE)

```


**Überprüfung des HHI-Werts**

Ich versuche zu interpretieren, was der HHI-Wert hier bedeutet, da er normalerweise mit der Summe der quadrierten Gewichtungen multipliziert mit 10.000 berechnet wird. Aber hier ist er einfach als "0,0036" angegeben, mit einer Standardabweichung von 0,005. Es scheint, dass der Wert des HHI anzeigt, wie konzentriert die Gewichtungen sind, das passt zu einer Diversität von 639,2588. Ich möchte sicherstellen, dass die Berechnungen korrekt sind und dass alles auf die Portfolio-Diversität hinweist.


I applied a three-step post-processing pipeline to tame the wild, unconstrained weights—and here’s what happened:

1. **Clamp & long-only**
   By forcing every stock’s weight into \[0%,10%] and re-scaling to fully invest, the new min/max weights are **0% / 9.36%**, with **zero** short positions.

2. **Smooth turnover**
   An exponential smoothing (factor = 0.2) cut the monthly choppiness: the average per‐stock weight change fell to about **0.11 %** (SD ≈ 0.05 %), and overall turnover plunged to roughly **0.31** per month.

3. **Diversification cap**
   Targeting an HHI of 0.3 drove concentration way down—mean HHI is now **0.0036** (SD ≈ 0.005), diversity jumped accordingly, and the L₁/L₂ norms of the weight vector are a modest **1.18** (SD ≈ 0.48) and **0.052** (SD ≈ 0.03), respectively.

In short, `postprocessing_portfolios()` turned our ±100 % network into a fully-invested, long-only, well-diversified, low-turnover strategy—exactly the kind of “production-ready” portfolio I need before re-evaluating performance.
...
tt <- summary.performance2(combined_portfolios_pp, transaction_cost = 0.005, gamma = 5, test=TRUE, print=TRUE)
...
combined_portfolios_pp <- postprocessing_portfolios(combined_portfolios, pp_config)
tt <- summary.performance2(combined_portfolios, transaction_cost = 0.01, gamma = 3, test=TRUE, print=TRUE)
```
...
save(combined_portfolios_pp, file = "combined_portfolios_pp.RData")
