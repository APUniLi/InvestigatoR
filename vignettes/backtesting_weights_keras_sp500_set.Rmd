...
config_constrained_shallow$layers[[3]]$activation <- activation_box_sigmoid(min_weight = -0.2, max_weight = 0.2)
config_constrained_shallow$loss$transaction_costs <- 0.005

config_constrained_deep <- config_unconstrained_deep
config_constrained_deep$layers[[3]]$activation <- activation_box_sigmoid(min_weight = -0.2, max_weight = 0.2)
...
# Run the backtest
pf_config9_shallow <- list(
  keras_regularized_bm_ir2_shallow = list(
    weight_func = "keras_weights_new",
    config = config_regularized_ir2_shallow
  )
)
# Run the backtest with multiple Keras configurations
keras_portfolios9_shallow_net <- backtesting_weights_new(
  data = sp500_m_signals,
  return_label = return_label,
  benchmark_label = "bm_new",
  mask_label = "sp500_flag_new",
  features = features,
  pf_config = pf_config9_shallow,  # Portfolio config with multiple Keras configurations
  # portfolio_object = keras_portfolios8,
  rolling = TRUE,
  window_size = window_size,
  step_size = step_size,
...
save(keras_portfolios9_shallow_net, file = "keras_portfolios9v3_new_bm_shallow.RData")
